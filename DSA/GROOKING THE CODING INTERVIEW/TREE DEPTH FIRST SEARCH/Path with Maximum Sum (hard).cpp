...
private:
    static int findMaximumPathSumRecursive(TreeNode *currentNode, int &globalMaximumSum)
    {
        ...
        int maxPathSumFromLeft =
            findMaximumPathSumRecursive(currentNode->left, globalMaximumSum);
        int maxPathSumFromRight =
            findMaximumPathSumRecursive(currentNode->right, globalMaximumSum);
