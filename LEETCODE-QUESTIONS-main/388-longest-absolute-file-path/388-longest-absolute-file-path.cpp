//https://leetcode.com/problems/longest-absolute-file-path/discuss/1241061/C%2B%2B-Easiest-O(n)-solution-with-explanation-Beats-100-0ms
	...
	int lengthLongestPath(string input){
         ...
         while(getline(ss, data, '\n'))
         {
		    // data will contain string like dir, /tsubdir1, /tsubdir2....etc
             ...
             // cout << numTabs << " -->" << data << ",  dataSize: "<< data.size() << ",   data.substrSize: " << data.substr(numTabs).size() << endl;
             ...
             if(numTabs == 0)
			    ...
			    // length upto previous dir + len("/") + no of chars after tabs for current filename/ dirname
                 // cout << data.substr(numTabs).size() << endl; 
