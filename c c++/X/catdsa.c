...
int Q[N],Pr[N];
int r = -1,f = -1;
void enqueue(char c,int p)//Enqueue function to insert data and its priority in queue
{
	int i;
...
int dequeue() //remove the data from front
{
	...
	{
		printf("deleted Element = %c\t Its Priority = %d",Q[f],Pr[f]);
		if(f==r)
			f = r = -1;
		else
			f++;
